...
func taintJPrint(taint_memory *TaintMemory, taint_stack *TaintStack) {
	fmt.Println()
	taint_stack.JPrint()
	taint_memory.JPrint()
}
...
func opReturn(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack, taint_memory *TaintMemory, taint_stack *TaintStack) ([]byte, []int, error) {
	...
	if taint_flag&OVERFLOW_FLAG > 0 {
		fmt.Println("return: overflow")
	} else if taint_flag&POTENTIAL_OVERFLOW_FLAG > 0 {
		fmt.Println("return: potential overflow")
	} else {
		fmt.Println("return: safe")
	}
...
func opRevert(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack, taint_memory *TaintMemory, taint_stack *TaintStack) ([]byte, []int, error) {
	...
	if taint_flag&OVERFLOW_FLAG > 0 {
		fmt.Println("revert: overflow")
	} else if taint_flag&POTENTIAL_OVERFLOW_FLAG > 0 {
		fmt.Println("revert: potential overflow")
	} else {
		fmt.Println("revert: safe")
	}
